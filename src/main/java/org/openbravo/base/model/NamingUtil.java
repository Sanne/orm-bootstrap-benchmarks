...
public class NamingUtil {
	 ...
	 * @param name
	 *            the name to check
	 * @return true if the name contains a char from the
	 *         {@link #ILLEGAL_ENTITY_NAME_CHARS}).
	 */
	...
	/**
	 * Checks a name for non-normal characters. Non-normal is everything from a to
	...
	public static boolean doesNameContainNonNormalCharacters(String name) {
		final char[] chars = name.trim().toCharArray();
		for (char c : chars) {
			final boolean normalChar = ('A' <= c && c <= 'Z') || ('0' <= c && c <= '9') || ('a' <= c && c <= 'z')
					|| c == '_';
			if (!normalChar) {
				return true;
			}
	 ...
	 * It has a performance benefit if the String constant is used because then
	 * HashMap get operations are faster (as equals also tests on object equality).
	 * This method tries to retrieve the String constant from the passed class. The
	 * clz parameter maybe null (happens for dynamic entities). In this case the
	 * passed propertyName is returned.
	 * 
	 * @param clz
	...
	public static String getStaticPropertyName(Class<?> clz, String propertyName) {
		try {
			...
			// see issue:
			// 14619: Generate entities do not notice that a field name has changed one
			// letter to upper
	...
	static String getPropertyMappingName(Property property) {
		...
		final boolean coreModuleProp = property.getModule() != null && !property.getModule().getId().equals("0");
		final boolean coreModuleEntity = false; // property.getEntity().getModule().getId().equals("0");
	...
	private static String stripIllegalCharacters(String value) {
		...
		for (char ch : value.toCharArray()) {
			...
			if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (!first && ch >= '0' && ch <= '9')) {
				result.append(ch);
				first = false;
			} else {
				result.append("");
	...
	private static boolean columnNameSameAsKeyColumn(Property p) {
		...
		return p.getColumnName().toLowerCase().equals(p.getReferencedProperty().getColumnName().toLowerCase());
	}
